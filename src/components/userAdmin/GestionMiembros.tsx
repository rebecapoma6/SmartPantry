...
export default function GestionMiembros() {
  ...
  // Generamos el link de invitación basado en el ID de la familia actual
  ...
  const cargarMiembros = async () => {
    ...
    setCargando(true);
    try {
      // OJO: Aquí hacemos un JOIN entre profiles y user_roles para traer el nombre y el rol
      ...
      if (data) {
        const miembrosFormateados = data.map((m: any) => ({
          id: m.id,
          nombre: m.nombre,
          avatar_url: m.avatar_url,
          rol: m.user_roles?.[0]?.role || 'Usuario'
        }));
